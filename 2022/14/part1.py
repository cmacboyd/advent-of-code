...
to_add = sorted([(x[0]-x_min, x[1]) for x in to_add])

# draw the starting grid
cols = range(0, x_max+1-x_min)
rows = range(0, y_max+1)
...
def drop_sand(grid, ssy, ssx):
	...
	# if off board we are done - return false
	if i == len(grid)-1 or j == len(grid[0])-1 or i == 0 or j ==0:
		return False
	# otherwise sand is at rest, update grid and return True
	else:
		grid[i][j] = 'O'
		return True
